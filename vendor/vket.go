...
import (
	...
	"github.com/valeriugold/vket/vs3"
...
func UploadSignPOST(w http.ResponseWriter, r *http.Request) {
	...
	base64Policy, s3Signature, err := vs3.GetSignedPolicy("us-east-1", AWSSecretAccessKey, buf.Bytes())
	if err != nil {
...
func UploadResultSuccess(w http.ResponseWriter, r *http.Request) {
	...
	vlog.Trace.Printf("calling SaveUploadedFile k=%s, evid=%s eid=%d, md5=%s\n", key, eventID, eid, md5[1:33])
	if err = vfiles.SaveUploadedFile(eid, name, key, md5[1:33]); err != nil {
		vlog.Warning.Printf("err on SaveMultipart, err:%v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
