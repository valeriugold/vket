...
// SignPolicyV4 signes the policy with V4
func SignPolicyV4(policy []byte, aWSSecretAccessKey, aWSRegion string) (base64Policy, s3Signature string, err error) {
	// func SignPolicy(region string, awsSecretAccessKey string, policy []byte) (base64Policy, s3Signature string, err error) {
	// func SignPolicy(region string, awsSecretAccessKey string, p io.Reader) (base64Policy, s3Signature string, err error) {
	service := "s3"
	base64Policy = ""
	s3Signature = ""
	...
	signingKey := getSignatureKey("AWS4"+aWSSecretAccessKey, dateStamp, aWSRegion, service)
...
// func main() {
// 	base64Policy, s3Signature, err := SignPolicy("us-east-1", vsecret.AWSSecretAccessKey, policy)
// 	if err != nil {
